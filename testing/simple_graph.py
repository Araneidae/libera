...
def GetCompensation(ioc, f_if=F_IF):
...
#         self.i = 0
    ...
    def Process(self, rawK, rawIQ):
        ...
        print 180/pi * angle(C)
        print abs(C)

        nIQ = IQ / mean(IQ)
        t = arange(nIQ.shape[1])

        qwtlab.axes(self.axes[0])
        qwtlab.plotarray2(abs(nIQ))
...
def MakeAxes():
    ...
    hbox = qt.QHBox(vbox)
    hbox2 = qt.QHBox(vbox)
    ax = [qwt.QwtPlot(hbox) for i in range(3)]
    ax.append(qwt.QwtPlot(hbox2))
    grid = qt.QGrid(2, qt.Qt.Horizontal, hbox2)
...
def MakeAxes_():
    widget = qt.QWidget()
    ax = [qwt.QwtPlot(widget) for i in range(4)]
...
def MakeAxes_():
    ...
    sublayout = qt.QGridLayout()
    layout.addLayout(sublayout, 1, 1)
    for n, a in enumerate(ax[4:]):
        sublayout.addWidget(a, n / 2, n % 2)

#     layout.addMultiCellWidget(ax[4], 2, 2, 0, qt.Qt.AlignCenter)
